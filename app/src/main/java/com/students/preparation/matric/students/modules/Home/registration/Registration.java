...
//Create a database connection
    ...
    public void registerStudent() {
        Toast.makeText(getBaseContext(),password.getText(),Toast.LENGTH_LONG).show();
        if (!validate()) {
    ...
    public boolean validate() {
        ...
        if (_school.isEmpty() || _school.length() < 3) {
            ...
            Toast.makeText(getBaseContext(),"school Error",Toast.LENGTH_LONG).show();
            valid = false;
        } else {
        ...
        if (role.equals("")){
            ...
            Toast.makeText(getBaseContext(),"Pass Error",Toast.LENGTH_LONG).show();
            valid = false;
        }
